...
Here we start to get into the mean-variance relationships and how they can be addressed. Limma-trend is a modification to the standard limma approach that incorporates mean expression level as a covariate into the prior hyperparameter estimation. Its goal is to adjust for any mean-variance relationship still leftover after transformation using log CPM. If we *don't* do this, we may be shrinking some genes too much, or too little, depending on their variance levels. Limma-trend is robust if the sequencing depth is "reasonably consistent" across samples (according to the user manual, less than 3-fold range, which is not quite the case here - see below). The way limma-trend works is the mean expression level is used as a covariate in the prior hyperparameter estimation.
